...
mean_data_t_series_df = mean_data_t_series_df.rename(columns={f"{pollutant} concentration": f"{pollutant} concentration ({units_dict[pollutant]})"})
print(mean_data_t_series_df.info())
window_length = [50, 200, 730]
mean_data_t_series_df[f'{window_length[0]}-day moving average'] = mean_data_t_series_df[f"{pollutant} concentration ({units_dict[pollutant]})"].rolling(window=window_length[0]).mean()
mean_data_t_series_df[f'{window_length[1]}-day moving average'] = mean_data_t_series_df[f"{pollutant} concentration ({units_dict[pollutant]})"].rolling(window=window_length[1]).mean()
mean_data_t_series_df[f'{window_length[2]//365}-yr moving average'] = mean_data_t_series_df[f"{pollutant} concentration ({units_dict[pollutant]})"].rolling(window=window_length[2]).mean()
print(mean_data_t_series_df.info())
sns.lineplot(x=mean_data_t_series_df.index, y=f"{pollutant} concentration ({units_dict[pollutant]})", data=mean_data_t_series_df, alpha=0.5, linewidth=0.5, label="Data")
